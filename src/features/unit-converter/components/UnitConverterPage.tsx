...
export function UnitConverterPage() {
  ...
  const convert = useCallback((sourceValue: string, from: string, to: string) => {
  ...
  return (
    <Card className="animate-fade-in-up stagger-4 backdrop-blur-md ring-border">
      <CardHeader>
        ...
        <CardDescription>
          Convert between units of measurement. Runs locally — nothing is uploaded.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <Label className="flex items-center justify-between text-sm font-medium">
            Precision
            <span className="text-muted-foreground text-xs">{precision} decimals</span>
          </Label>
          <input
            type="range"
            min="0"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-3">
            ...
            <Select value={fromUnit} onValueChange={(value) => handleUnitChange(value, toUnit)}>
              <SelectTrigger>
            ...
            <Textarea
              ...
              className="min-h-[100px] resize-y font-mono text-lg text-center"
            />
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-border">
          ...
          <p className="text-xs text-muted-foreground">
            Category: <span className="font-medium">{CATEGORY_LABELS[category]}</span>
            {fromUnit && toUnit && (
              <> | <span className="font-mono">{fromUnit} → {toUnit}</span></>
            )}
          </p>
        </div>
      </CardContent>
