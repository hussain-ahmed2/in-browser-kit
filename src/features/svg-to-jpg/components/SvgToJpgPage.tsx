...
export function SvgToJpgPage() {
  ...
  const { svgContent, scale, backgroundColor, status, resultBlobUrl, fileName } = useAppSelector(
    (s) => s.svgToJpg
  )
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      const blob = await convertSvgToJpg(svgContent, scale, backgroundColor)
      const url = URL.createObjectURL(blob)
      dispatch(conversionDone(url))
      toast.success('JPG created!')
    } catch (err) {
      console.error(err)
      dispatch(conversionError())
      toast.error('Failed to convert SVG to JPG.')
    }
  }, [svgContent, scale, backgroundColor, dispatch])

  const handleDownload = () => {
    if (!resultBlobUrl) return
    const a = document.createElement('a')
    a.href = resultBlobUrl
    a.download = (fileName?.replace(/\.svg$/i, '') || 'converted') + '.jpg'
    a.click()
    ...
    <>
      ...
      <Card className="animate-fade-in-up stagger-4 backdrop-blur-md ring-border">
        <CardHeader>
          ...
          <CardDescription>
            Convert SVG files to JPG images with configurable background color.
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 p-4 rounded-xl bg-secondary/30 border border-border">
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                onChange={(e) => dispatch(backgroundColorSet(e.target.value))}
            ...
            <div className="flex gap-2">
              ...
              >
                {status === 'converting' ? (
                  ...
                  <><Image className="w-4 h-4 mr-2" /> Convert to JPG</>
                )}
              </Button>
          ...
          {status === 'done' && resultBlobUrl && (
            <div className="space-y-4 animate-fade-in">
              <div className="p-6 bg-green-500/10 border border-green-500/30 rounded-xl text-center">
                <p className="text-green-600 dark:text-green-400 font-medium">JPG Ready!</p>
              </div>
              <div
                className="flex justify-center border border-border rounded-xl p-4"
                style={{ backgroundColor }}
              >
                <img src={resultBlobUrl} alt="Converted JPG" className="max-w-full max-h-96" />
              </div>
