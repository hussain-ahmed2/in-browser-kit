...
self.onmessage = async (e: MessageEvent<WorkerInputMessage>) => {
    ...
    try {
        ...
        console.log(
            "✅ WebCodecs Initialization Successful! (The warning above is normal because we are resizing the video, so it falls back to the rerender path intentionally). Starting execution...",
        );
        ...
        conversion.onProgress = (progressRatio) => {
            ...
            const logs = `frame=${frames.toString().padStart(4, " ")} fps=${fps
                .toString()
