...
import { Code, Copy, Check, Download, RotateCcw } from 'lucide-react'
...
import { inputSet, outputSet, clearAll } from '../htmlMinifierSlice'
import { minifyHtml, compareSizes } from '../lib/htmlMinifier'

export function HtmlMinifierPage() {
  const dispatch = useAppDispatch()
  const { input, output } = useAppSelector((s) => s.htmlMinifier)
  ...
  return (
    <Card className="animate-fade-in-up stagger-4 backdrop-blur-md ring-border">
      <CardHeader>
        ...
        <CardDescription>
          Remove comments, collapse whitespace, and minify HTML for production.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 p-4 rounded-xl bg-secondary/30 border border-border">
          <div className="flex gap-2">
            ...
            >
              <Code className="w-4 h-4 mr-2" /> Minify
            </Button>
