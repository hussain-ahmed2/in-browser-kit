...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full glass border-b border-border/50">
      {/* Bottom glow line */}
      <div className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-brand/40 to-transparent" aria-hidden="true" />
      ...
      <div className="container mx-auto flex h-14 max-w-screen-2xl items-center px-6">
        ...
        <nav className="hidden sm:flex items-center gap-1.5">
          {navLinks.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href} className="rounded-full">
                <Button
        ...
        <div className="flex flex-1 items-center justify-end gap-1">
              ...
              return (
                <Link key={href} href={href}>
                  <Button
                    ...
                    className={cn(
                      "rounded-full",
                      isActive && "ring-1 ring-brand/30 bg-brand/10 text-brand"
                    )}
                  >
          ...
          {mounted && (
            <Button variant="ghost" size="icon-sm" onClick={toggleTheme} aria-label="Toggle theme" className="rounded-full">
              {theme === "dark" ? (
